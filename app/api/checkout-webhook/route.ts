...
export async function POST(req: Request, response: NextResponse) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      if (event.data.object.metadata!.courseId) {
        await axios.patch(`https://e-learning-eight-tau.vercel.app/api/courses/${event.data.object.metadata!.courseId}`, {
          buyers: [event.data.object.metadata!.userId]
        })
      }
    } catch (error) {
