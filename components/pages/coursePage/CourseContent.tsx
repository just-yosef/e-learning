...
export default function CourseContent({ snippet }: { snippet: YouTubeSearchResultItem[] }) {
    ...
    return (
        <>
            ...
            {/* @ts-ignore */}
            <BuyCourseButton courseId={snippet[0]?.id!} imgs={snippet.slice(0, 5).map((item) => item.snippet.thumbnails.medium.url)} courseTitle={snippet[0].snippet.title} price={10000} courseDescription="No" />
        </>
    );
}
