...
const ExploreCourses = () => {
    ...
    const dropDownsData = [
        {
            title: "Subject",
            content: ["كيمياء", "لغة عربية", "لغة انجليزية", "تاريخ", "جغرافيا", "علوم"]
        },
        {
            title: "Level",
                ...
                {dropDownsData.map((el) => (
                    <DropdownMenu >
                        ...
                        <DropdownMenuContent align="end">
                            {el.content.map((e, i) => (
                                <DropdownMenuItem onClick={(item) => {
                                    const key = sParams.get(el.title.toLowerCase());
                                    updateSingleParam(el.title.toLowerCase(), ((item.target) as HTMLElement).textContent)
